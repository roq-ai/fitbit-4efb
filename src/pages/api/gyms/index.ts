...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createGym() {
    ...
    const body = { ...req.body };
    if (body?.workout_session?.length > 0) {
      const create_workout_session = body.workout_session;
      body.workout_session = {
        create: create_workout_session,
      };
    } else {
      delete body.workout_session;
    }
